...
class ClockDialPainter extends CustomPainter{
  ...
  void paint(Canvas canvas, Size size) {
    var tickMarkLength;
    final angle= 2* pi / 60;   // the clock has 60 minutes on it's face. figure out the angle in radians of each tick mark on the ui face here
    ...
    canvas.translate(radius, radius);   // this is setting the origin point to the center rather than top left
    ...
    for (var i = 0; i< 60; i++ ) {
        ...
        // if it's a minute divisible by 5 (5min, 10min, 15min)...then this tick mark will be longer  
      ...
      // if the current loop index is divisable by 5, we are on a hour tic mark.  we have to draw a text label (the numeral on the clock face) for it.
      if (i%5==0){
        ...
        canvas.translate(0.0, -radius+20.0);      // move the origin of painting to the "1" position, the first time thru when i==0.
                                                  // this is the just inside the hour tic mark.  that's where we will start laying the numerals down
        ...
        textPainter.layout();   // allow the layout to be computed.  you have to call this but I can't explain why.  yet.
  ...
  bool shouldRepaint(CustomPainter oldDelegate) {
    // there is nothing else in the app other than the timer tick at each 1 second that would trigger a repaint, so we always return false.  
    return false;
  }
}
